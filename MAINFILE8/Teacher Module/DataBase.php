    ...
    function idverify()
    {
        try{
            ...
            if(!$conn1){
                ...
                $userid=$_POST["userid"];
                $result1 = mysqli_query($conn1, "SELECT t_email, t_email_time,t_pass FROM teacher WHERE t_id=$userid");
                $query_row=mysqli_fetch_array($result1);
                if(mysqli_num_rows($result1)==0){
                    $return_data['status'] ="no";
                }else if($query_row["t_pass"] == null){
                    $return_data['passnotset'] ="no";
                }else{
                    date_default_timezone_set('ASIA/KOLKATA');
                    $last_date=strtotime($query_row["t_email_time"]);
                    $current_date = strtotime("now");
                    if(($last_date + 172800) <= $current_date){
                        $_SESSION["oldDate"]=$query_row["t_email_time"];
                        $otp=rand(100000,999999);
                        $to =$query_row["t_email"];
        ...
        try{
            ...
            if(!$conn1){
                ...
                if($otp==$otpmail){
                    ...
                    mysqli_query($conn1,"UPDATE teacher SET t_email_time='$current_date' WHERE t_id=$userid");
                }else{
                    $return_data['status'] = "no";
                }
                echo json_encode($return_data);
    ...
    function otpval()
    {
        try{
            ...
            if(!$conn1){
                ...
                date_default_timezone_set('ASIA/KOLKATA');
                $current_date = (date("Y-m-d") ." ". date("H:i:s"));
                $result1 = mysqli_query($conn1, "SELECT t_pass FROM teacher WHERE t_id=$userid");
                if(mysqli_num_rows($result1)>0){
                    ...
                    if($row1["t_pass"]==$pass){
                        ...
                        mysqli_query($conn1,"UPDATE teacher SET t_pass='$pass' WHERE t_id= $userid");
