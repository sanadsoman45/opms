...
$s_id=$_POST["s_id"];
$t_id=$_POST["t_id"];
$stud_query=mysqli_query($conn1,"SELECT * from college_database.stud_table where s_id=$s_id");
$teacher_query=mysqli_query($conn1,"SELECT * from college_database.teac_table where t_id=$t_id");
...
$query_teacher = mysqli_query($conn1, "SELECT * from college_database.teac_table where t_id NOT IN (SELECT t_id from projectsubmission.teacher) order by t_id");
if(mysqli_num_rows($stud_query)<=0)
{
	$return_data["name"]="sname";
}

if(mysqli_num_rows($teacher_query)<=0)
{
	$return_data["name"]="tname";
}

if(mysqli_num_rows($query_teacher)>0)
{
	while($rows=mysqli_fetch_array($query_teacher))
	{
		if($rows["t_name"]==$t_name)
		{ 
			$return_data["name"]="tname";
		}
	}
